...
'''
'FDG' stands for feature-driven graph
'''
...
class GraphPropagate(nn.Module):
    ...
    def forward(self, x, graph):
        ...
        for k in range(1, self.Ks):
            x_k = torch.einsum("bij,btjc->btic", graph, x_k.clone())
...
class STGformer(nn.Module):
    def __init__(
        ...
        self.road_graph_repres = supports[0].float() # physical road representation
        ...
        self.structure_learner = GraphLearner(self.model_dim, self.model_dim)
    ...
    def forward(self, x):
        ...
        # print (f"--debug graph shape {self.road_graph_repres.shape} x shape {x.shape} --")
        graph = self.structure_learner(x, self.road_graph_repres) # learn the graph through physical road network
