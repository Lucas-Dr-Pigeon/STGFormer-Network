...
from model.STSEAGWNN import STSEAGWNN
...
def train_one_epoch(
    ...
    for x_batch, y_batch in train_bar:
        ...
        # for n,p in model.temporal_graph_generator.named_parameters():
        #     print(n, p.grad is None, p.grad.norm() if p.grad is not None else None)

        # Entropy loss
        entropy_loss = -(A_time * (A_time + 1e-12).log()).sum(-1).mean()
        # Off-diagonal loss
        eye = torch.eye(A_time.size(-1), device=A_time.device)
        offdiag_loss = (A_time * (1 - eye)).mean()

        loss_task = criterion(out_batch, y_batch)
        batch_loss_list.append(loss_task.item())

        loss = loss_task + 1e-2*entropy_loss + 1e-3*offdiag_loss
        ...
        # with torch.no_grad():
        #     print("A_time row 0:", A_time[0,0].cpu().numpy())
        #     print("Mean:", A_time.mean().item(),
        #         "Std:", A_time.std().item(),
        #         "Diag mean:", A_time.diagonal(dim1=-2, dim2=-1).mean().item())

        train_bar.set_description(f"train task loss:{loss_task.item():.5f} entropy loss:{entropy_loss.item():.5f} offdiag loss:{offdiag_loss.item():.5f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--dataset", type=str, default="STSEAGWNN_INRIX_MANHATTAN")
    ...
    model = partial(STSEAGWNN, num_nodes=num_nodes, adj=adj_mx[0], device=DEVICE)
    # print (f"----debug DEVICE overwrite issue {DEVICE}----")
    ...
    # Build dummy input on the same device
    # dummy_x = torch.randn(
    #     cfg["batch_size"],
    #     cfg["in_steps"],
    #     cfg["num_nodes"],
    #     next(iter(trainset_loader))[0].shape[-1],
    #     device=DEVICE,   # 👈 align to model
    # )

    # summary(
    #     model,
    #     inputs=dummy_x,   # use actual tensor, not input_size
    #     verbose=0,
    # )
    # print_log(log=log)
