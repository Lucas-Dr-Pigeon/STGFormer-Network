...
class DirectedLowRankEdgeScorer(nn.Module):
    ...
    def __init__(self, in_dim: int, hidden: int, rank_r: int, L: int,
                 pooling: str = "last", mix: str = "diag", nonneg: bool = False, dropout: float = 0.2):
        super().__init__()
        ...
        # Dropout for low-rank embeddings (applied after src/dst MLP)
...
class PropSTGformer(nn.Module):
    def __init__(
        ...
        self.temporal_proj_dyn  = nn.Conv2d(self.base_model_dim, self.model_dim, (1, kernel_size[0]), 1, 0)
        ...
        self.pos_prop_graph = DirectedLowRankEdgeScorer(
            self.model_dim, hidden=64, rank_r=1, L=num_lags, pooling="last", mix="diag"
        )
    ...
    def forward(self, x):
        ...
        x_dyn = self.temporal_proj_dyn(base_cat.transpose(1, 3)).transpose(1, 3)   # (B,T,N,C_full)
        ...
        for attn in self.attn_layers_s:
            x_0 = attn(x_0, G_adp)   # consistent API
