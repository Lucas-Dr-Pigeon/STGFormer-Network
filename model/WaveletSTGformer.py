...
class SafeLayerNorm(nn.Module):
    def __init__(self, normalized_shape, eps=1e-5, clamp_val=1e3, cast_fp32=True):
        super().__init__()
    ...
    def forward(self, x):
        # Replace NaN/Inf early and clamp magnitude
        x = torch.nan_to_num(x, nan=0.0, posinf=self.clamp_val, neginf=-self.clamp_val)
        x = x.clamp(-self.clamp_val, self.clamp_val)
        # Do LN in fp32 for stability
        if self.cast_fp32 and x.dtype != torch.float32:
            y = self.ln(x.float())
            return y.to(x.dtype)
        return self.ln(x)
...
class FastAttentionLayer(nn.Module):
    ...
    def forward(self, x, edge_index=None, dim=0):
        ...
        if x.size(1) > 1:
            ...
            # Sanity check params before using them
            # if not torch.isfinite(out).all():
            #     raise RuntimeError("[NaN/Inf] input to out_proj already bad")

            W, b = self.out_proj.weight, self.out_proj.bias
            # print("out_proj.weight stats:", W.min().item(), W.max().item(), W.std().item())
            # if b is not None:
            #     print("out_proj.bias stats:", b.min().item(), b.max().item())
            # if not torch.isfinite(W).all():
            #     raise RuntimeError("[NaN/Inf] out_proj.weight itself bad")
            # if b is not None and not torch.isfinite(b).all():
            #     raise RuntimeError("[NaN/Inf] out_proj.bias itself bad")
    ...
    def fast_attention(self, x, qs, ks, vs, dim=0):
        qs = nn.functional.normalize(qs, dim=-1, eps=1e-6)
        ks = nn.functional.normalize(ks, dim=-1, eps=1e-6)

        # if torch.isnan(qs).any():
        #     print("[NaN in qs after normalize]")
        #     raise RuntimeError("NaN in qs")
        # if torch.isnan(ks).any():
        #     print("[NaN in ks after normalize]")
        #     raise RuntimeError("NaN in ks")

        N = qs.shape[1]
        b, l = x.shape[dim : dim + 2]

        kvs = torch.einsum("blhm,blhd->bhmd", ks, vs)
        # if torch.isnan(kvs).any():
        #     print("[NaN in kvs]")
        #     raise RuntimeError("NaN in kvs")

        attention_num = torch.einsum("bnhm,bhmd->bnhd", qs, kvs)
        # if torch.isnan(attention_num).any():
        #     print("[NaN in attention_num]")
        #     raise RuntimeError("NaN in attention_num")
        ...
        # if torch.isnan(attention_normalizer).any():
        #     print("[NaN in attention_normalizer]")
        #     raise RuntimeError("NaN in attention_normalizer")

        attention_normalizer = attention_normalizer.unsqueeze(-1)
        attention_normalizer += torch.ones_like(attention_normalizer) * N

        out = attention_num / attention_normalizer.clamp_min(1e-5)
        # if torch.isnan(out).any():
        #     print("[NaN in final attention out]")
        #     raise RuntimeError("NaN in final attention out")
...
class GraphWaveletLayer(torch.nn.Module):
    ...
    def forward(self, input, phi_matrices, phi_inverse_matrices):
        # input: (B, T, N, C)
        B, T, N, C = input.shape
        x = input.permute(0, 3, 2, 1)  # (B, C, N, T)

        S = phi_matrices.shape[0]

        # Diagonal weights (squash them a bit to avoid explosions)
        W = 10.0
        diag_w = W * torch.tanh(self.diag_weights / W)
        diag = torch.diag_embed(diag_w)  # (S, N, N)

        # φ @ diag @ φ⁻¹
        phi_product = phi_matrices @ diag @ phi_inverse_matrices
        # if torch.isnan(phi_product).any() or torch.isinf(phi_product).any():
        #     raise RuntimeError("[NaN/Inf] phi_product in GraphWaveletLayer")

        # Flatten input
        xv = x.permute(0, 1, 3, 2).reshape(-1, N)  # (B*C*T, N)

        # Wavelet transform
        x_trans = torch.einsum('sij,bj->sbi', phi_product, xv)
        # if torch.isnan(x_trans).any() or torch.isinf(x_trans).any():
        #     raise RuntimeError("[NaN/Inf] x_trans in GraphWaveletLayer")

        # Aggregate across scales
        h = x_trans.permute(1, 0, 2).sum(dim=1)  # (BCT, N)
        # if torch.isnan(h).any() or torch.isinf(h).any():
        #     raise RuntimeError("[NaN/Inf] h aggregation in GraphWaveletLayer")

        # Reshape back
        h = h.reshape(B, C, T, N).permute(0, 1, 3, 2)
        h = h.permute(0, 3, 2, 1)  # (B, T, N, C)

        h = F.dropout(h, self.dropout, training=self.training)
        # if torch.isnan(h).any() or torch.isinf(h).any():
        #     raise RuntimeError("[NaN/Inf] output h in GraphWaveletLayer")
    ...
    #     diag = torch.diag_embed(torch.clamp(self.diag_weights, -10, 10))  # (S, N, N)
...
class SelfAttentionLayerWavelet(nn.Module):
    def __init__(
        ...
        self.ln1 = SafeLayerNorm(model_dim, eps=1e-5, clamp_val=1e3, cast_fp32=True)
        self.ln2 = SafeLayerNorm(model_dim, eps=1e-5, clamp_val=1e3, cast_fp32=True)
        self.ln_att_in = SafeLayerNorm(model_dim, eps=1e-5, clamp_val=1e3, cast_fp32=True)
        self.ln_mlp_in = SafeLayerNorm(model_dim, eps=1e-5, clamp_val=1e3, cast_fp32=True)
        self.ln_wavelet = SafeLayerNorm(model_dim, eps=1e-5, clamp_val=1e3, cast_fp32=True)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x, phi_matrices, invesre_phi_matrices):
        print (f"--Debug attention layer x.shape {x.shape}--")
        # ---- Pre-LN for attention path ----
        x_att_in = self.ln_att_in(torch.nan_to_num(x, nan=0.0, posinf=1e3, neginf=-1e3).clamp(-1000.0, 1000.0))

        # Local (wavelet) stack
        x_loc = self.locals(x_att_in, phi_matrices, invesre_phi_matrices)  # list: [x_att_in, wavelet(x_att_in), ...]
        c = x_att_in
        x_delta = 0.0

        for i, z in enumerate(x_loc):
            # Sanitize + normalize wavelet features before attention
            z = torch.nan_to_num(z, nan=0.0, posinf=1e3, neginf=-1e3).clamp(-50.0, 50.0)
            z = self.ln_wavelet(z)

            att_outputs = self.attn[i](z)  # FastAttentionLayer already hardened

            # if not torch.isfinite(att_outputs).all():
            #     raise RuntimeError(f"[NaN/Inf] att_outputs at order {i}")

            # FiLM-style gated residual (bounded)
            # map c -> (gamma, beta) with small gain for stability
            proj = self.pws[i](c)
            # Split last dim if you prefer affine; here we just gate multiplicatively
            gate = torch.sigmoid(proj).clamp(0.0, 1.0)
            upd  = (att_outputs * gate).clamp(-20.0, 20.0)

            alpha = (0.05 if i == 0 else self.scale[i])
            x_delta = x_delta + alpha * upd
            c = att_outputs

        # Residual join -> sanitize BEFORE ln1 to avoid LN NaNs
        y = x + self.dropout(x_delta)
        y = torch.nan_to_num(y, nan=0.0, posinf=1e3, neginf=-1e3).clamp(-100.0, 100.0)
        x = self.ln1(y)  # SafeLayerNorm (fp32 math)

        # ---- MLP block (pre-LN) ----
        x_mlp_in = self.ln_mlp_in(x)
        mlp_out = self.fc(x_mlp_in)
        x = x + self.dropout(mlp_out)

        # Final light LN
        x = self.ln2(x)
        # if not torch.isfinite(x).all():
        #     raise RuntimeError("[NaN/Inf] after ln2 in SelfAttentionLayerWavelet")
        return x
...
class WaveletSTGformer(nn.Module):
    ...
    def forward(self, x):
        ...
        #     print("[NaN BEFORE encoder_proj]", x.min().item(), x.max().item())
        #     raise RuntimeError("NaN found before encoder_proj")
