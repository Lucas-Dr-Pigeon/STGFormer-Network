...
class noflayer(nn.Module):
    ...
    def process_adj(self, A_TNN):
        """ A_TNN: (T,N,N) """
        if self.nonneg:
            A_TNN = torch.sigmoid(A_TNN)
        if self.symmetrize:
            A_TNN = 0.5*(A_TNN + A_TNN.transpose(-2, -1))
        return A_TNN

    def attention(self, x_BTNC, A_TNN):
        """
        x_BTNC: (B,T,N,C)
        A_TNN: (T,N,N)
        returns U,P: (B,T,N,N)
        ...
        # A_TNN = self.process_adj(A_TNN)         # (T,N,N)
        A_BTNN = A_TNN.unsqueeze(0).expand(B,-1,-1,-1)  # (B,T,N,N)
    ...
    def forward_lifting_bases(self, x_BTNC, P_BTNN, U_BTNN, A_BTNN):
        B,T,N,C = x_BTNC.shape
        coe = torch.sigmoid(self.temp)
        cheb_coe = torch.sigmoid(self.cheb)

        AdjP = A_BTNN * P_BTNN          # (B,T,N,N)
    ...
    def forward(self, input, A_TNN, _format="BTNC"):
        """
        input: (B,N,T,C_in)
        A_TNN: (T,N,N) adjacency per time step
        """
        if _format == 'BCNT':
            x_BTNC = input.permute(0,3,2,1).contiguous() # (B,T,N,C)
        elif _format == 'BNTC':
            x_BTNC = input.permute(0,2,1,3).contiguous()
        else:
            x_BTNC = input

        U,P,A_BTNN = self.attention(x_BTNC, A_TNN)
        out_BTNC = self.forward_lifting_bases(x_BTNC, P, U, A_BTNN)

        if _format == 'BCNT':
            out = out_BTNC.permute(0,3,2,1).contiguous() # (B,N,T,C)
        elif _format == 'BNTC':
    ...
    # # ---------- Forward ----------
    # def forward(self, input, A, _format='BTNC'):
    #     """
    #     input: (B,N,T,C_in)
    ...
    #     returns: (B,N,T,C_out)
    #     """
    #     print (f"--Debuf noflayer input.shape {input.shape}")
    #     if _format == 'BCNT':
...
class GraphWaveletPropagate(nn.Module):
    def __init__(self, model_dim, num_nodes, order=2, dropout = 0.2):
        super(GraphWaveletPropagate, self).__init__()
        self.wavelet_layer = noflayer(num_nodes, model_dim, model_dim, hop=3, alpha=None,
                 residual=False, variant=False, leaky_alpha=0.2, alp=0.9)
        self.order = order
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x, adj):
        ...
        for k in range(1, self.order):
            x_k = self.wavelet_layer(x_k.clone(), adj)
            x_list.append(x_k)
        return x_list
...
class SelfAttentionLayerWavelet(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.locals = GraphWaveletPropagate(model_dim, num_nodes, order, dropout)
        self.attn = nn.ModuleList(
            [
                FastAttentionLayer(model_dim, num_heads, mask, kernel=kernel)
                for _ in range(order)
    ...
    def forward(self, x, adj):
        ...
        # Local (wavelet) stack
        x_loc = self.locals(x_att_in, adj)  # list: [x_att_in, wavelet(x_att_in), ...]
        c = x_att_in
        x_delta = 0.0
