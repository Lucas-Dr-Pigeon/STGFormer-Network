...
def train_one_epoch(
    model, trainset_loader, optimizer, scheduler, criterion, clip_grad, log=None, A_mask=None, lags=[], alpha=0.1, scaler=None
):
    ...
    for x_batch, y_batch in train_bar:
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):
            x_batch = x_batch.to(DEVICE)
            y_batch = y_batch.to(DEVICE)
            out_batch, _pos_logits, _neg_logits = model(x_batch)
            ...
            loss_1 = criterion(ov, yv)
            # loss_1 = torch.tensor(0, device=DEVICE)
            loss_2 = prop_graph_bce_loss(_pos_logits, _neg_logits, x_batch, y_batch, SCALER, A_mask, lags, device=DEVICE)
        ...
        scaler.update()
        train_bar.set_description(f"train loss:{loss.item():.5f} {loss_1.item():.5f} {loss_2.item():.5f} bad count:{bad_mask.sum().item()}/{B} empty count:{(~non_empty_mask).sum().item()}/{B*N}")
...
def train(
    ...
    torch.set_float32_matmul_precision("high")  # for matmuls
    scaler = torch.cuda.amp.GradScaler(enabled=True)

    for epoch in range(max_epochs):
        train_loss = train_one_epoch(
            model, trainset_loader, optimizer, scheduler, criterion, clip_grad, log=log, A_mask=A_mask, lags=lags, scaler=scaler
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--dataset", type=str, default="PROPSTGFORMER_INRIX_MANHATTAN")
