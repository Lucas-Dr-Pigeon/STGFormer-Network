...
class MultiScaleGraphPropagate(nn.Module):
    """
    Message passing over K-1 hops using a (multi-lag) directed graph in sparse edge form.

    Inputs:
      x:           (B, T, N, F)      node features over time
      edge_index:  (2, E)            fixed topology (src,dst) with nodes in [0..N-1]
      graph_ble:   (B, L, E)         per-batch, per-lag edge weights or logits
      weights:     (optional) per-lag combiner:
                   - if combine='weighted': (L,) or (B, L)
                     a softmax over L is applied, then a weighted sum over lags.

    Returns:
      x_list: list [x^(0), x^(1), ..., x^(K-1)], each shape (B, T, N, F)

    Notes:
      - No Top-K here (as requested).
      - Row-normalization is optional; off by default (set normalize_rows=True if needed).
      - Uses a block-diagonal sparse mm to batch all graphs in one SpMM per hop.
    """
    def __init__(self,
                 Ks: int,
                 dropout: float = 0.2,
                 combine: Literal["mean", "sum", "weighted"] = "mean",
                 fuse_time: bool = True,
        ...
        self._idx_cache = {}  # (device,B,N,E,hash) -> (rows, cols, size)

    @staticmethod
    def _pool_lags_sparse(graph_ble: torch.Tensor,
                          combine: str,
                          weights: Optional[torch.Tensor]) -> torch.Tensor:
        """
        graph_ble: (B, L, E)  -> returns (B, E)
        """
        B, L, E = graph_ble.shape
        if combine == "mean":
            return graph_ble.mean(dim=1)                          # (B,E)
        if combine == "sum":
            return graph_ble.sum(dim=1)                           # (B,E)
        # weighted
        if weights is None:
            raise ValueError("combine='weighted' requires 'weights' in forward().")
        if weights.ndim == 1:
            w = torch.softmax(weights, dim=0).view(1, L, 1)       # (1,L,1)
            return (graph_ble * w).sum(dim=1)                     # (B,E)
        if weights.ndim == 2:
            if weights.shape[:2] != (B, L):
                raise ValueError(f"weights must be (B,L); got {tuple(weights.shape)}")
            w = torch.softmax(weights, dim=1).unsqueeze(-1)       # (B,L,1)
            return (graph_ble * w).sum(dim=1)                     # (B,E)
        raise ValueError("weights must be (L,) or (B,L)")

    @staticmethod
    def _row_softmax(edge_index: torch.Tensor,
                     scores_bE: torch.Tensor,
                     num_nodes: int,
                     eps: float = 1e-9) -> torch.Tensor:
        """
        Sparse row-wise softmax over edges sharing the same source node.
        edge_index: (2,E) with edge_index[0]=src
        scores_bE : (B,E) (typically logits)
        returns   : (B,E) normalized weights
        """
        B, E = scores_bE.shape
        src = edge_index[0]  # (E,)
        # row-wise max
        max_bN = scores_bE.new_full((B, num_nodes), -1e30)
        max_bN.scatter_reduce_(1, src.expand(B, -1), scores_bE, reduce="amax", include_self=True)
        centered = scores_bE - max_bN.gather(1, src.expand(B, -1))
        w = torch.exp(centered.clamp_max(20.0))
        # row-wise sum
        sum_bN = torch.zeros_like(max_bN)
        sum_bN.scatter_add_(1, src.expand(B, -1), w)
        return w / (sum_bN.gather(1, src.expand(B, -1)) + eps)

    @torch.no_grad()
    def _blockdiag_indices(self,
                           edge_index: torch.Tensor,
                           B: int,
                           N: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Size]:
        """
        Build (and cache) block-diagonal COO indices for B graphs of size N×N.
        """
        device = edge_index.device
        E = edge_index.size(1)
        src, dst = edge_index[0], edge_index[1]
        h = int(src.sum().item()) ^ (int(dst.sum().item()) << 1) ^ (E << 8) ^ (N << 16)
        key = (device, B, N, E, h)
        if self.cache_indices and key in self._idx_cache:
            return self._idx_cache[key]
        offset = torch.arange(B, device=device).view(B, 1) * N
        rows = (dst.view(1, -1) + offset).reshape(-1).contiguous()   # (B*E,)
        cols = (src.view(1, -1) + offset).reshape(-1).contiguous()   # (B*E,)
        size = torch.Size([B * N, B * N])
        if self.cache_indices:
            self._idx_cache[key] = (rows, cols, size)
        return rows, cols, size

    def _spmm_blockdiag(self,
                        x: torch.Tensor,            # (B,T,N,F)
                        edge_index: torch.Tensor,   # (2,E)
                        w_bE: torch.Tensor          # (B,E)
                        ) -> torch.Tensor:          # (B,T,N,F)
        B, T, N, F = x.shape
        device, dtype = x.device, x.dtype
        rows, cols, size = self._blockdiag_indices(edge_index, B, N)
        vals = w_bE.reshape(-1).to(device=device, dtype=dtype)

        A_bd = torch.sparse_coo_tensor(
        ...
        if self.fuse_time:
            X = x.permute(0, 2, 1, 3).contiguous()  # (B,N,T,F)
            X = X.view(B * N, T * F)                # (B*N, T*F)
            Y = torch.sparse.mm(A_bd, X)            # (B*N, T*F)
            Y = Y.view(B, N, T, F).permute(0, 2, 1, 3).contiguous()
        else:
            Y = torch.empty_like(x)
            for t in range(T):
                Xt = x[:, t].reshape(B * N, F)      # (B*N, F)
                Yt = torch.sparse.mm(A_bd, Xt)      # (B*N, F)
                Y[:, t] = Yt.view(B, N, F)
        return Y
    ...
    def forward(self,
                x: torch.Tensor,              # (B, T, N, F)
                edge_index: torch.Tensor,     # (2, E)
                graph_ble: torch.Tensor,      # (B, L, E)  (weights or logits)
        ...
        if graph_ble.ndim != 3:
            raise ValueError(f"graph_ble must be (B,L,E); got {tuple(graph_ble.shape)}")

        B, T, N, F = x.shape
        # 1) combine lags in edge space -> (B,E)
        # inside MultiScaleGraphPropagate.forward(...)
        if graph_ble.dim() == 2:             # (B,E) pre-pooled fast-path
            w_bE = graph_ble                 # (B,E)
        elif graph_ble.dim() == 3:           # (B,L,E)
            w_bE = self._pool_lags_sparse(graph_ble, self.combine, weights)  # (B,E)
        else:
            raise ValueError("graph_ble must be (B,E) or (B,L,E)")

        # 2) optional sparse row-normalization
        if self.normalize_rows:
            w_bE = self._row_softmax(edge_index, w_bE, num_nodes=N)      # (B,E)

        # 3) multi-hop propagation (sparse block-diagonal SpMM)
        x_k = x
        x_list = [x]  # hop-0
        for _ in range(1, self.Ks):
            x_k = self._spmm_blockdiag(x_k, edge_index, w_bE)            # (B,T,N,F)
            x_list.append(self.dropout(x_k))
        return x_list
...
class SelfAttentionLayer(nn.Module):
    def __init__(
        ...
        # sparse multiscale knobs (used only when multiscale=True):
        ms_fuse_time: bool = True,          # fuse T into one SpMM per hop (fast; matches dense einsum behavior)
        ms_combine: str = "mean",           # {"mean","sum","weighted"} for lag combine when (B,L,E) is passed
        ms_normalize_rows: bool = False,    # set True if graph_ble are logits and you want sparse row-softmax inside
        ms_cache_indices: bool = True,
    ):
        super().__init__()
        self.kernel = kernel
        self.order = order
        ...
        # --- Local propagation backends ---
        if not multiscale:
            # Dense/static path (original)
            gso = supports[0] if (supports is not None and len(supports) > 0) else None
            self.locals_dense = GraphPropagate(Ks=order, gso=gso)
            self.locals_sparse = None
        else:
            # Sparse/dynamic path over (edge_index, graph_ble[, weights])
            self.locals_sparse = MultiScaleGraphPropagate(
                Ks=order,
                dropout=dropout,
                combine=ms_combine,
                fuse_time=ms_fuse_time,
                normalize_rows=ms_normalize_rows,
                cache_indices=ms_cache_indices,
            )
            self.locals_dense = None  # not used in multiscale=True layers

        # --- Per-hop fast attention + 1x1 ---
        ...
        for i in range(order):
            nn.init.constant_(self.pws[i].weight, 0.0)
            nn.init.constant_(self.pws[i].bias, 0.0)
        ...
        # Scale per hop (first hop full, later hops small residual)
        self.scale = [1.0] + [0.01] * (order - 1)

    # ----- helpers -----
    def _locals_dense_call(self, x: torch.Tensor, graph: torch.Tensor) -> List[torch.Tensor]:
        # graph: (B,N,N) or (B,L,N,N)  -> GraphPropagate returns list len==order
        if not isinstance(graph, torch.Tensor) or graph.dim() not in (3, 4):
            raise TypeError("Dense SelfAttentionLayer expects graph as torch.Tensor of shape (B,N,N) or (B,L,N,N).")
        return self.locals_dense(x, graph)

    def _locals_sparse_call(
        self,
        x: torch.Tensor,
        graph: Union[Tuple[torch.Tensor, torch.Tensor], Tuple[torch.Tensor, torch.Tensor, torch.Tensor], dict],
    ) -> List[torch.Tensor]:
        # graph: (edge_index, graph_ble[, weights]) or dict with same keys
        if isinstance(graph, (tuple, list)):
            if len(graph) == 2:
                edge_index, graph_ble = graph
                weights = None
            elif len(graph) == 3:
                edge_index, graph_ble, weights = graph
            else:
                raise ValueError("Sparse graph must be (edge_index, graph_ble[, weights]).")
        elif isinstance(graph, dict):
            edge_index = graph.get("edge_index", None)
            graph_ble = graph.get("graph_ble", None)
            weights   = graph.get("weights", None)
        else:
            raise TypeError("Sparse graph must be a (edge_index, graph_ble[, weights]) tuple/list or dict.")

        if not (isinstance(edge_index, torch.Tensor) and edge_index.dim() == 2 and edge_index.size(0) == 2):
            raise ValueError("edge_index must be a tensor of shape (2,E).")
        if not (isinstance(graph_ble, torch.Tensor) and graph_ble.dim() in (2, 3)):
            raise ValueError("graph_ble must be a tensor of shape (B,E) or (B,L,E).")

        # MultiScaleGraphPropagate supports (B,E) fast-path (pre-pooled) and (B,L,E)
        return self.locals_sparse(x, edge_index, graph_ble, weights=weights)

    def _call_locals(self, x: torch.Tensor, graph) -> List[torch.Tensor]:
        if not self.multiscale:
            return self._locals_dense_call(x, graph)
        else:
            return self._locals_sparse_call(x, graph)
    ...
    def forward(self, x: torch.Tensor, graph):
        ...
        x:     (B,T,N,C)
        graph: 
          - if multiscale=False: torch.Tensor (B,N,N) or (B,L,N,N)
          - if multiscale=True : (edge_index, graph_ble[, weights]) where
                edge_index: (2,E), graph_ble: (B,E) or (B,L,E)
        """
        # Local multi-hop propagation (list length == order)
        x_loc = self._call_locals(x, graph)  # each z in x_loc is (B,T,N,C)

        # Hop-wise fast-attention with residual mixing
        c = x_glo = x
        for i, z in enumerate(x_loc):
            att_outputs = self.attn[i](z)                         # (B,T,N,C)
...
class PropSTGformer(nn.Module):
    def __init__(
        ...
        self.attn_layers_s = nn.ModuleList(
            [
                SelfAttentionLayer(
                    ...
                    kernel=size, supports=supports, multiscale=False,
                )
        ...
        self.attn_layers_d1 = nn.ModuleList(
            [
                SelfAttentionLayer(
                    self.model_dim, mlp_ratio, num_heads, dropout,
                    kernel=size, supports=supports, multiscale=True,
                )
                for size in kernel_size
            ]
        )
    ...
    def forward(self, x):
        ...
        w_pos = torch.sigmoid(logits_p).mean(dim=1)  # (B,E)
        w_neg = torch.sigmoid(logits_n).mean(dim=1)  # (B,E)

        x_1, x_2 = x_full.clone(), x_full.clone()
        for attn in self.attn_layers_d1:
            x_1 = attn(x_1, (self.edge_index, w_pos))   # pass (B,E), skip lag-pool inside
        for attn in self.attn_layers_d2:
            x_2 = attn(x_2, (self.edge_index, w_neg))
