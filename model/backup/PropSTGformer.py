...
class SelfAttentionLayer(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.locals = GraphPropagate(Ks=order, gso=supports[0]) if not multiscale \
            else MultiScaleGraphPropagate(Ks=order)
...
#         self.pos_prop_graph = DirectedLowRankEdgeScorer( self.model_dim, hidden=64, rank_r=1, \
#                                                         L=num_lags, pooling="last", mix="diag" )
#         self.neg_prop_graph = DirectedLowRankEdgeScorer( self.model_dim, hidden=64, rank_r=1, \
#                                                         L=num_lags, pooling="last", mix="diag")
...
#             x_0 = attn(x_0, graph_0)     # consistent API

#         # ---- Dynamic graphs (per lag), then average over lags ----
        
#         logits_p = self.pos_prop_graph(x, self.edge_index)  # (B,L,E)
#         logits_n = self.neg_prop_graph(x, self.edge_index)  # (B,L,E)

#         A_pos = edges_to_sparse_coo(logits_p, self.edge_index, self.num_nodes) # sparse graph
#         A_neg = edges_to_sparse_coo(logits_n, self.edge_index, self.num_nodes) # sparse graph
#         # print (f"--debug A_pos shape: {A_pos.to_dense().shape} A_neg shape: {A_neg.to_dense().shape} --")
#         x_1, x_2 = x.clone(), x.clone()
#         for attn in self.attn_layers_d1:
#             x_1 = attn(x_1, A_pos.to_dense())
#         for attn in self.attn_layers_d2:
#             x_2 = attn(x_2, A_neg.to_dense())
...
class PropSTGformer(nn.Module):
    ...
    def forward(self, x):
        ...
        for attn in self.attn_layers_d1:
            x_1 = attn(x_1, A_pos.to_dense())   # keep API consistent with your attn
        for attn in self.attn_layers_d2:
            x_2 = attn(x_2, A_neg.to_dense())
