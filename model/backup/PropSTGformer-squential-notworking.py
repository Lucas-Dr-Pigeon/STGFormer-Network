...
class PropSTGformer(nn.Module):
    def __init__(
        ...
        self.attn_layers_d1 = nn.ModuleList(
            [
                SelfAttentionLayer(
                    self.base_model_dim, mlp_ratio, num_heads, dropout,
    ...
    def forward(self, x):
        ...
        A_pos_dense = edges_to_sparse_coo(logits_p, self.edge_index, self.num_nodes).to_dense().contiguous()
        A_neg_dense = edges_to_sparse_coo(logits_n, self.edge_index, self.num_nodes).to_dense().contiguous()
        
        x_1, x_2 = x_dyn.clone(), x_dyn.clone()
        for attn in self.attn_layers_d1:
            x_1 = attn(x_1, A_pos_dense)   # keep API consistent with your attn
        for attn in self.attn_layers_d2:
            x_2 = attn(x_2, A_neg_dense)

        x = (x_1 + x_2).contiguous()  
        ...
        if self.adaptive_embedding_dim > 0:
            ...
            x = torch.cat([x, self.dropout(adp_emb)], dim=-1)
        ...
        # --- before static attention: break all aliasing on both inputs ---
        x = (x.contiguous() + 0.0)                # fresh storage, keeps grads
        G_adp_safe = (G_adp.contiguous() + 0.0)   # in case attn edits the bias in place
